...
namespace TC.Functions.Patients.Business
{
    public class InitialDiagnosisCardBusiness : BaseBusiness<FormDiagInicials, PatientContext>, IInitialDiagnosisCardBusiness
    {
        IMapper mapper;
        int UserId = 12;
        public InitialDiagnosisCardBusiness(PatientContext context, IPrincipal userInfo, IConfiguration configuration) : base(context, userInfo, configuration)
        {
        ...
        public Result<FormDiagResult> GetFormById(GetFormById dto)
        {
            ...
            if (result == null)
            {
                return Result<FormDiagResult>.SetError("No se encontraron registros");
            }
